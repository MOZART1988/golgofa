...
class CustomVariables extends LanguageActiveRecord
{
    ...
    public static function getOrCreate($title, $defaultValue)
    {
        $exists = self::find()->where(['title' => $title])->exists();

        if ($exists) {

            $item = self::find()->where(['title' => $title, 'is_active' => 1])->one();
